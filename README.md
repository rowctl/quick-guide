...
```markdown
 ...
 | Key  | Action                  | What It Does                                                                  |
 |------|-------------------------|-------------------------------------------------------------------------------|
 | F9   | Safe Transaction Run    | Wraps current query in `BEGIN TRANSACTION …` with manual COMMIT/ROLLBACK.     |
 | F10  | Rollback                | Inserts `ROLLBACK;` — safe exit, no changes saved.                            |
 | F11  | Commit                  | Inserts `COMMIT;` — finalize changes explicitly.                              |
 | F12  | Full Safe Execute       | Adds full block: TRY/CATCH with auto‑COMMIT on success, ROLLBACK on error.    |
